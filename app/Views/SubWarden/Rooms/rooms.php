  ...
  ?>
    ...
    <script>
      ...
      function searchAstudent() {
        $('#othermembers').hide(100);
        var input = document.getElementById("myInput").value;
        if (input == "") {
          alert("Enter a valid key to search");
        } else {
          var genof = <?php echo json_encode($gen); ?>;
          if (genof == "male") {
            $.ajax({
              url: "SubWarden_Rooms/searchForAStu",
              method: "POST",
              data: {
                detail: input,
                gender: genof
      ...
      function sendsmstoparents(x, y) {
        var stid = document.getElementById('idofStu').value;
        var g = <?php echo json_encode($gen); ?>;
        $.ajax({
          url: "SubWarden_Rooms/sendsmstoparents",
          method: "POST",
          data: {
            room: y,
            parentNo: x,
        ...
        var det = document.getElementById('nameofstu').value;
      ...
      window.onload = function() {
        ...
        //alert(genof);
        $.ajax({
          url: "SubWarden_Rooms/getDetProgress",
          method: "POST",
          data: {
            gen: genof
          },
    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="col-md-1 my-4">
          <button class="btn btn-primary" onclick="searchAstudent()" data-toggle="popover" title="Search" data-content="Search by student ID"><i class="fas fa-search"></i></button>
        </div>

      </div>
      <div class="row">
        <div class="col-md-5">
          <div class="studentDataSet">
          </div>
        </div>
        <div class="col-md-6 othermembers" id="othermembers" style="display:none;left:2%;">
    ...
    <div class="container cate" id="mapDiv" style="padding-top:5%">
      ...
      <div class="container">
        <div class="container" style="display:block;" id="theFloorSelection">
          <div class="row" style="color:black">
            <div class="col-sm text-center">
              <input type="radio" class="theFloorRadio" name="floor"><button class="theBut" type="button" id="1" onclick="floorNo(this.id)" data-toggle="popover" title="Floor selection" data-content="Click a floor to view rooms">FLOOR 1</button>
            </div>
            <div class="col-sm text-center">
              <input type="radio" class="theFloorRadio" name="floor"><button type="button" class="theBut" id="2" onclick="floorNo(this.id)" data-toggle="popover" title="Floor selection" data-content="Click a floor to view rooms">FLOOR 2</button>
            </div>
            <div class="col-sm text-center">
              <input type="radio" class="theFloorRadio" name="floor"><button class="theBut" type="button" id="3" onclick="floorNo(this.id)" data-toggle="popover" title="Floor selection" data-content="Click a floor to view rooms">FLOOR 3</button>
            </div>
            <div class="col-sm text-center">
              <input type="radio" class="theFloorRadio" name="floor"><button class="theBut" type="button" id="4" onclick="floorNo(this.id)" data-toggle="popover" title="Floor selection" data-content="Click a floor to view rooms">FLOOR 4</button>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="row">
              <div class="row theMap">
                <div class="row" style="width:100%;">
                  <div class="rightLine" style="display: flex" data-toggle="popover" title="More Details" data-content="Click a room to view details">
        ...
        <div class="modal-content">
          ...
          <script>
            ...
            function showAllDataS() {
              $("#moreDetS").toggle(1000);
              $('.iconForMore').show(500);
              if ($(this).text() == "HIDE") {
                $(this).text("VIEW MORE");
              } else {
                $(this).text("HIDE");
            ...
            function showMoreRoomMembers(x) {
              var genof = <?php echo json_encode($gen); ?>;
              $.ajax({
                url: "SubWarden_Rooms/othersinRoom",
                method: "POST",
                data: {
                  id: x,
                  gender: genof
    ...
    <script>
      ...
      function addNewStudent() {
        ...
        if (genof == "male") {
          $.ajax({
            url: "SubWarden_Rooms/add_boys_list",
          ...
          $.ajax({
            url: "SubWarden_Rooms/add_girls_list",
      ...
      function assignRoomStudent() {
        var genof = <?php echo json_encode($gen); ?>;
        var st_id = document.getElementById("stID").value;
        var room = document.getElementById("roomAdd").value;
        var floor = String(room)[0];
        if (genof == "male") {
          $.ajax({
            url: "SubWarden_Rooms/addStudentNew",
            method: "POST",
            data: {
              st_id: st_id,
              floor: floor,
          ...
          $.ajax({
            url: "SubWarden_Rooms/addGirlStudentNew",
            method: "POST",
            data: {
              st_id: st_id,
              floor: floor,
      ...
      function setRegDate() {
        var level = document.getElementById("levelReg").value;
        $.ajax({
          url: "SubWarden_Rooms/getRegDate",
          method: "POST",
          data: {
            level: level
          },
          success: function(data) {
            $('#setDateSet').html(data);
          },
          error: function(data) {
      ...
      function setDateLimit(x) {
        var level = x;
        var endD = document.getElementById('endDate').value;
        $.ajax({
          url: "SubWarden_Rooms/setRegEndDate",
          method: "POST",
          data: {
            level: level,
            Enddate: endD
    ...
    <div class='container'>
      ...
      <button class="btn btn-success" style="margin-top:5%" type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalAddStudent" onclick="addNewStudent()" data-toggle="popover" title="Assign a room for student" data-content="Assign a room for student">ADD STUDENT</button>
      <button class="btn btn-success" style="margin-top:5%" type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalRegDate" data-toggle="popover" title="Set Registration Date" data-content="Set Registration Date">SET REGISTRATION DATE</button>

    </div>
    <div class="modal fade" id="exampleModalAddStudent" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
          ...
          <div class="modal-body" id="addNewStuSet">
            <form>
              <div id="studentSet"></div>
              <small id="emailHelp" class="form-text text-muted">Select a Room</small>
              <input type="number" name="room" id="roomAdd" min="101" max="428" placeholder="00" class="form-control">
              <div class="col  d-flex justify-content-end">
                <button type="reset" class="btn btn-danger m-1">RESET</button>
                <button type="button" class="btn btn-success m-1" name="addNewSt" onClick="assignRoomStudent()">SUBMIT</button>
              </div>
            </form>
          </div>
    ...
    <div class="modal fade" id="exampleModalRegDate" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">SET REGISTRATION DATE</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body" id="addNewStuSet">
            <form>
              <small id="emailHelp" class="form-text text-muted">Select a Room</small>
              <select class="form-control" name="levelReg" onchange="setRegDate()" id="levelReg">
    ...
    <br>
    <form id="roomChangeSt" style="display:none" class="">
      <div class="row">
