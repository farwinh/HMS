...
class Dean_Notices extends BaseController
{
        ...
        foreach(($result=$obj_notice->findAll()) as $row){
            ...
            if($f!=""){
                ...
                <tr >
                    ...
                    <td>'.$row['time'].'</td>
                    <td>'.$row['title'].'</td>
                    <td ><div id="div'.$row['notice_id'].'" type="button"  onclick="myFunctionView(this.id)">View More</div></td>                    
                </tr>
    ...
    public function getMyNoticesAfter($user){
        $output='';
        $obj_notice=new notice_model();
        //$user=$this->request->getVar('sender');
        foreach(($result=$obj_notice->where('owner',$user)->findAll()) as $row){
    ...
    public function DelNotices(){
        ...
        if($result){
            $table=$this->getMyNoticesAfter($user);
            return $table;
        }else{
            $table=$this->getMyNoticesAfter($user);
            return $table;
